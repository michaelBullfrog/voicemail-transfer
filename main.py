...
async def receive_voicemail_transfer(
    ...
    # Log exactly what Flow Designer sent before attempting to parse it.
    logger.info(
        "Raw request body received from Flow Designer: %s",
        raw_body,
    )
    print(
        "RAW REQUEST BODY:",
    ...
    if raw_body.strip():
        ...
        json_parse_error = "Request body was empty."

    if json_parse_error:
        logger.error(
            "Invalid JSON received from Flow Designer | "
            "error=%s | rawBody=%s",
            json_parse_error,
            raw_body,
        )
        ...
        last_request = {
            ...
            "emailSent": False,
            "error": "Invalid JSON received from Flow Designer.",
            "jsonParseError": json_parse_error,
            "rawBody": raw_body,
        }

        # Return 200 so a malformed notification payload does not
        # interfere with the caller being transferred to voicemail.
        return {
            "accepted": True,
            "emailSent": False,
            "error": "Invalid JSON received from Flow Designer.",
    ...
    if not isinstance(parsed_payload, dict):
        logger.error(
            "Flow Designer body was valid JSON but not an object | "
            "payload=%s",
            parsed_payload,
        )
